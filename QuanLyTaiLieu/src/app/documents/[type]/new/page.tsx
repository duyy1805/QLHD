...
export default async function NewDocumentPage({ params }: { params: Promise<{ type: string }> }) {
  ...
  return <DashboardLayout><div className="space-y-5"><div><h1 className="text-2xl font-semibold">Thêm {docType.name}</h1><p className="text-sm text-muted-foreground">Mọi người dùng đăng nhập đều có thể upload tài liệu.</p></div><DocumentForm type={docType} users={users} /></div></DashboardLayout>;
}
