    ...
    try {
        const result = await new sql.Request(transaction)
            ...
            .query(`
                ...
                SELECT @PackageExists = 1, @CurrentLocationId = ID_ViTriKho
                FROM dbo.TheKhoKienBTP WITH (UPDLOCK, HOLDLOCK)
                WHERE ID_TheKhoKienBTP = @ID_TheKhoKienBTP_QROnly AND TonTai = 1;

                IF @PackageExists = 0
                    SELECT -2 AS StatusCode, N'Không tìm thấy kiện' AS Message;
                ELSE IF EXISTS
                (
                    SELECT 1 FROM dbo.TheKhoKienBTP WITH (UPDLOCK, HOLDLOCK)
                ...
                BEGIN
                    ...
                    SELECT 1 AS StatusCode, N'Cập nhật QR thành công' AS Message,
...
// ID bang 0 trong du lieu BTP cu co nghia la chua xac dinh. Cho phep khop
// voi ID day du tu ERP, nhung van tru han muc de khong tinh lap so luong.
const importQuantitiesFit = (allowedRows, requestedRows, keys, allowedQuantityKey, requestedQuantityKey) => {
    const requested = [...groupQuantity(requestedRows, keys, requestedQuantityKey).entries()]
        ...
        .sort((left, right) => right.values.filter(Boolean).length - left.values.filter(Boolean).length);
